...
public class frmQLLoaiPhong extends JPanel implements ActionListener{
	...
	public frmQLLoaiPhong() throws MalformedURLException, NotBoundException, RemoteException {
		...
		//table
		String column[] = {"Mã loại phòng", "Tên loại phòng","Số Giường","Giá Ngày" };
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(o == btnThem)
		{
            popupThemLoaiPhong gdThemPhong = null;
            try {
                gdThemPhong = new popupThemLoaiPhong();
		...
		if(o == btnCapNhat)
		{
			...
			if(row != -1)
			{
				popupCapNhatLoaiPhong gdCapNhat = new popupCapNhatLoaiPhong();
				gdCapNhat.setLocationRelativeTo(null);
                                
                               
				gdCapNhat.txtTenLoaiPhong.setText((String) tblModel.getValueAt(row, 1));
				gdCapNhat.txtSoGiuong.setText((String)tblModel.getValueAt(row, 2));
				gdCapNhat.txtGia.setText((String)tblModel.getValueAt(row, 3));
	...
	public void showDataTable() throws MalformedURLException, NotBoundException, RemoteException {
		...
		for (LoaiPhong p : ds) {
                        String[] row = {
                            Integer.toString(p.getMaLoaiPhong()),
                            p.getTenLoaiPhong(),
                            String.valueOf(p.getSoGuong()),
                            String.valueOf(p.getGiaNgay())
                        };
                        tblModel.addRow(row);
		}
